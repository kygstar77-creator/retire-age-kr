...
import { logoutClearLocal } from '../../utils/firemapStateApi.js';
  ...
  if (acc && acc.handle) {
    return (
      <div className="fm-idline">
        ...
        <button type="button" className="fm-idline-link" onClick={async () => { await logoutClearLocal(); window.location.reload(); }}>로그아웃</button>
      </div>
    );
  }
  return (
