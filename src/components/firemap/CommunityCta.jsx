...
export default function CommunityCta({ where = 'result' }) {
  ...
  return (
    <Card padding="md">
      ...
      <ListGroup>
        <ListRow lead="🟢" title="파이어맵 네이버 카페" href={CAFE_URL} external onClick={() => hit('cafe_click')} />
        {OPENCHAT_URL && (
          <ListRow lead="💬" title="카카오톡 오픈채팅" href={OPENCHAT_URL} external onClick={() => hit('openchat_click')} />
        )}
      </ListGroup>
    </Card>
  );
