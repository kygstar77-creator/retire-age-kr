...
# 규칙(실측 근거):
#  - 제목 2줄, 윗줄 흰색 / 아랫줄 노란색. 굵은 제목체(Black Han Sans, OFL 무료).
...
def cfg(kind):
    ...
    base = {'long': {'text_y': 0.72, 'bg_bright': 0.45, 'panel_alpha': 150, 'yellow_bottom': 1, 'num_yellow': 1, 'stroke_ratio': 16},
            'short': {'text_y': 0.52, 'bg_bright': 0.30, 'panel_alpha': 150, 'yellow_bottom': 0, 'num_yellow': 1, 'stroke_ratio': 16}}[kind]
...
def make(top, bottom, out, bg=None, short=False, brand='파이어맵'):
    ...
    stroked(dr, ((W - dr.textlength(bottom, font=f2)) / 2, y0 + lh), bottom, f2, YELLOW if c.get('yellow_bottom', 1) else WHITE, sw)   # 쇼츠 상위는 노랑 면적이 오히려 적다(0.0067)
    fb = body(int(W * 0.028))
