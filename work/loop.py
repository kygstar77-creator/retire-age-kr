...
# 2026-09-26: 출력을 줄 단위로 흘린다. 전에는 기본(블록) 버퍼라 도는 동안 받아 둔 파일이
# 계속 0바이트였다. 그래서 '느린 바퀴'와 '죽은 바퀴'를 구별할 수가 없었다 — 이날 137회차를
# 회차가 "14:15쯤 죽었다"고 오진해 그 틀린 진단을 기록과 커밋에 적었다. 실제로는 703초 만에
# 정상으로 끝나 있었다. 한 바퀴가 6~10분 걸리므로 줄마다 흘려도 값은 무시할 만하고,
# 대신 어디까지 갔는지가 도는 중에도 보인다(죽으면 어디서 죽었는지도 남는다).
