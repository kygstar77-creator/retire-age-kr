...
# 하루에 몇 편까지. 카페는 매시 1편이라 24편이 정상이고 그 위로만 닫는다.
# 간격 20분만 있고 총량이 없어 산술적으로 72편까지 열려 있던 것을 막는 용도다.
#
# 한때 4편으로 조였다가 사장님 지시로 되돌렸다(2026-09-26) — "카페는 매일 24편 발행하는 거 그대로 해".
# 내가 근거를 잘못 붙였다. "많이 올려서 색인이 빠졌다"고 했는데 카페 색인은 멀쩡했다.
#   9/17  9편 발행 · 카페탭 색인 2/2      9/23 18편 · 2/2
#   9/22 10편 발행 · 카페탭 색인 2/2      9/24 21편 · 2/2
# 21편 올린 날도 색인된다. 처음 "0/8"로 본 것은 통합검색으로 재고 다음을 카페탭으로 잰 탓이다.
# 탭을 바꿔 놓고 날짜 차이로 읽었다.
#
...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            if cmd == 'check':
                ok = logged_in(page)
                # 발행 쪽(0.6시간)과 같은 창을 쓴다. 여기만 6시간이라 IP 차단이 한 번 나면
                # 풀린 뒤에도 check가 여섯 회차 동안 '로그인 안 됨'을 답했고, 회차 규칙이
                # "로그인 안 됨이면 발행 불가"라 원고가 그만큼 잠겼다(2026-09-26 13시 회차).
