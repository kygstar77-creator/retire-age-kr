...
import { CHALLENGES, QUOTES, QUICK, dayIdx, todayStr, yesterdayStr, wonStr, readJSON, fmtAdvance, dailyNeedOf, addSave } from '../../firemap-v2/dailyData.js';
...
export default function Savings({ simulation, onMove }) {
  ...
  const log = (amount) => setSv(addSave(amount));
  const custom = () => {
    ...
    <main className="fm-screen fm-scroll fm-has-tabbar">
      ...
      <section className="fm-card fm-save-screen">
        ...
        <div className="fm-save-chips" aria-label="오늘 아낀 항목 기록">
          {QUICK.map((q) => (
            <button type="button" key={q.label} onClick={() => log(q.won)}>
