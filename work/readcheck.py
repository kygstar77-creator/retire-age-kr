...
BODY_BAD = [
    # 2026-09-27: '늘어나는 셈이죠'가 빠져나갔다(앞 글자가 '하는'이 아니라 '나는'). 관형형 어미 전체로 넓혔다.
    (r'[인한된는운을]\s*셈이(죠|에요|예요|다|네요|고)', '"~인 셈이죠" — 말로는 잘 안 한다. 그냥 단정해서 쓴다'),
    (r'결론부터', '"결론부터 말씀드리면" — 보고서 말투. 바로 숫자로 시작한다'),
    (r'(사라|사지\s*마라|사지\s*말라)는?\s*(얘기|말)', '"사라·사지 마라는 얘기는 안 한다" — 안 한다는 말을 굳이 쓰지 않는다'),
    (r'판단은\s*(각자|독자|여러분)', '"판단은 각자" — 맺음말 군더더기'),
    (r'(짐작으로|추측으로)\s*적지\s*않', '안 한다는 말을 굳이 쓰지 않는다'),
