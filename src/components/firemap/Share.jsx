...
async function makeShareImage(inputs, simulation) {
  ...
  ctx.fillStyle = '#fff7ed'; ctx.font = 'bold 34px sans-serif'; ctx.fillText('생활비 · 수익률 · 국민연금까지 조건을 바꿔 비교', 120, 450);
...
export default function Share({ inputs, simulation, onBack }) {
  ...
  const shareText = `파이어맵 계산 결과\n${inputs.targetRetirementAge}세 퇴사 → ${runwayText(simulation)}까지\n생활비, 수익률, 국민연금 조건을 바꿔 비교해봤어요.`;
  const copyFallback = async (text, label) => { try { await navigator.clipboard.writeText(text); setMessage(label); setTimeout(() => setMessage(''), 1800); } catch { setMessage('공유가 막혀 있어요. 주소창에서 직접 복사해주세요.'); } };
  const shareLink = async (url, title) => {
    ...
    await copyFallback(url, '공유가 안 되어 링크를 복사했어요');
  };
  const shareImage = async () => {
    const blob = await makeShareImage(inputs, simulation); const file = new File([blob], 'firemap-result.png', { type: 'image/png' });
    if (navigator.canShare?.({ files: [file] })) { await navigator.share({ files: [file], title: '파이어맵 계산 결과', text: shareText }); setMessage('이미지 카드 공유창을 열었어요'); }
  ...
  return (
    <main className="fm-screen fm-scroll">
      ...
      <section className="fm-card fm-text-card"><p className="fm-kicker">공유</p><h2>내 FIRE 결과 공유하기</h2><p>이미지 카드나 링크 공유창으로 바로 보낼 수 있어요.</p><div className="fm-share-preview"><strong>{inputs.targetRetirementAge}세 퇴사 → {runwayText(simulation)}까지</strong><p>생활비, 수익률, 국민연금 조건을 바꿔 비교해봤어요.</p></div><button className="fm-primary" type="button" onClick={shareImage}>결과 이미지 공유하기</button><button className="fm-secondary" type="button" onClick={() => shareLink(BASE_URL, '파이어맵')}>앱 링크 공유하기</button><button className="fm-secondary" type="button" onClick={() => shareLink(shareUrl, '내 파이어맵 조건')}>내 조건 링크 공유하기</button><button className="fm-secondary" type="button" onClick={() => copyFallback(`${shareText}\n\n${BASE_URL}`, '결과 문구 복사됨')}>결과 문구 복사</button>{message && <div className="fm-toast">{message}</div>}</section>
