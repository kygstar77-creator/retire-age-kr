...
export default function InsightReport({ simulation }) {
  ...
  const messages = [
    survivesTargetAge
      ? `사용자가 입력한 종료 나이인 ${inputs.simulationUntilAge}세까지 금융자산이 고갈되지 않습니다. 이 기준에서는 ${inputs.targetRetirementAge}세 퇴사가 가능합니다.`
      : `${inputs.targetRetirementAge}세에 퇴사하면 ${targetResult.depletionAge}세에 금융자산이 고갈될 수 있어 입력한 ${inputs.simulationUntilAge}세까지 버티기 어렵습니다.`,
    ...
    earliestRetirementAge
      ? `${inputs.simulationUntilAge}세까지 고갈되지 않는 가장 빠른 퇴사 가능 나이는 ${formatAge(earliestRetirementAge)}입니다.`
