...
## work/calcheck.py — 계산 검산 (2026-09-25 17시 C 회차)
`py -3.12 work/calcheck.py <pkg>` · selfcheck.py 가 자동으로 부른다(지적은 `[검산]` 태그, 사실 지적과 같이 세어 발행을 막는다).

문장이 **말로 적어 둔 계산**을 실제로 다시 한다. 아홉 가지:
증감률("A에서 B로 … N% 늘었다") · 해 대 해("2022년과 2025년을 비교하면 N%") · %포인트 · 배수 ·
공제("N%를 떼면") · 나눗셈("B로 나누면") · 표 합계 · 표 평균 · 제목 숫자 대조 · 시세 날짜 대조.

헛짚지 않으려고 둔 규칙(전부 실측에서 나왔다):
- 어긋남이 30%를 넘으면 그 문장이 말하는 관계가 아니라고 보고 넘긴다.
- 표는 고정 창이 아니라 덩어리째 읽는다. 문장이 "열 건"이라고 했는데 표가 여덟 줄이면 검산하지 않는다.
- "1.5배 넘게" 같은 어림말은 검산 대상이 아니다.
- 반올림 여유가 단위를 따라간다 — '1.11억'의 ±0.005 는 0.005원이 아니라 50만원이다.
- 제목은 내려 적는 것을 허용한다(본문 41.94%포인트 → 제목 41%포인트).
- 날짜는 값이 붙은 시세 문장(종가·마감)만 본다. 지급일·공시일은 앞으로 올 일정이라 사실표에 없는 게 정상이다.
