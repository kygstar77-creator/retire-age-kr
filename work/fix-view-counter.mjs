...
function snippet(ctype, cid) {
  return `<style>.fm-vc{display:inline-flex;align-items:center;gap:5px;font-size:13px;color:#8e8e93;margin:8px 0 4px;font-weight:600}</style>` +
    ...
    `function show(n){var b=badge();if(typeof n==='number'&&!isNaN(n)){b.textContent='\\uD83D\\uDC41 '+fmt(n)+'\\uD68C \\uC870\\uD68C';}}` +
    `try{badge();fetch(U+'/rest/v1/rpc/fm_view_hit',{method:'POST',headers:{apikey:K,authorization:'Bearer '+K,'content-type':'application/json'},body:JSON.stringify({p_type:T,p_id:I})})` +
...
walk(ROOT);
console.log(`[fix-view-counter] injected view counter into ${count} pages`);
