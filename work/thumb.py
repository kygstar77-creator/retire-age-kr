...
def make(top, bottom, out, bg=None, short=False, brand='파이어맵'):
    ...
    if len(parts) == 3:
        ...
        # 윗줄만 키우는 손잡이(top_scale)는 91회차에 달았다가 재 보고 물렀다. 다시 달지 않는다.
        # 격자 6점(1.0~2.4)이 세 칸 값까지 전부 똑같았다 — top 0.2504 · mid 0.4771 · bot 0.3139, 오차 0.1912.
        # 이유: 윗줄 크기는 sz = min(세 줄 fit) 로 정해지는데, 루프 샘플 제목에서 윗줄이 가장 긴 줄이라
        # 그 min 을 만드는 것이 윗줄 자신이다(실측 fit: 금천 61% 강남 36% = 115 · 서울 전세가율 183 · 전수 조사 260).
    ...
    # split_scale 1.6, 줄높이 135/215/217, cy 963): spread 1.0 에서 윗줄 y=222·아랫줄 y=1570 으로
    # 위(76)·아래(1607) 어느 클램프에도 아직 안 닿는다. 진짜 한계는 아랫줄이 1.07, 윗줄이 1.39 에서
    # 클램프에 붙는 것이라 1.4 까지가 실제로 움직이는 구간이다. 그래서 윗한계를 1.4 로 넓힌다
    # (그 위는 두 클램프가 다 붙어 값이 안 변한다 — 격자를 더 늘려도 의미가 없다).
    # 넓히고 94회차에 격자 9점을 실제로 그려 재 봤다. 오차는 안 줄었다 — 1.0 이 진짜 최선이다:
    ...
    # 그래도 한계는 1.0 으로 되돌리지 않는다. 되돌리면 '그리기 한계에서 멈췄다'는 막힘이 회차마다
    # 다시 뜨는데, 그건 이제 사실이 아니다 — 1.0 은 격자가 짧아서가 아니라 재서 고른 최선이다.
    spread = min(max(float(c.get('text_spread', 0.0)), 0.0), 1.4)
    if len(parts) == 2:
