  ...
  return (
    <div className="ds-chart">
      ...
      <div className="ds-chart__canvas" style={{ touchAction: 'none' }} onPointerDown={pick} onPointerMove={(e) => { if (e.buttons) pick(e); }} onTouchStart={pick} onTouchMove={pick}>
        ...
        <span className="ds-chart__pt num" style={{ left: pctX(a0), top: pctY(pts[0].v) }}>{formatWon(pts[0].v)}</span>
        <span className="ds-chart__pt ds-chart__pt--end num" style={{ left: pctX(a1), top: pctY(pts[n - 1].v) }}>{formatWon(pts[n - 1].v)}</span>
      </div>
