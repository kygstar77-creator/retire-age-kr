...
def main():
    ...
    DIVSTOCK = ['리얼티인컴(O)', '코카콜라(KO)', '존슨앤존슨(JNJ)', '프록터앤드갬블(PG)', '알트리아(MO)',
                '버라이즌(VZ)', '펩시코(PEP)', '맥도날드(MCD)', '애브비(ABBV)', '셰브론(CVX)', '엑슨모빌(XOM)']  # C9
    ...
    def topic(sid, k):
        ...
        if sid == 'C9': return (f'{rot(DIVSTOCK, k)} 한 종목 분석(카페판: 배당·현금흐름 각도) — 배당 이력·배당률·배당성향·'
