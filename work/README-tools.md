...
## 마스터 루프 (2026-09-23) — 무엇을 고칠지도 스스로 정한다
- `work/health.py` 계기판: 발행(편수·0편 회차·대기 묶음)·품질(실물검증·교차검증)·검색(색인·순위)·성장(방문·카페 조회)·영상(조회·편수)·디자인(경쟁 대비 차이)·도구(대기열·사람 손) 16항목을 목표 대비로 재고 `점수 = 부족분 × 중요도`로 순위. → work/health.json
- `work/selfloop.py` 마스터 루프: 계기판 갱신 → 상위 5개를 일감으로 뽑아 `work/worklist.md`에 씀 → 기계로 되는 건 즉시 실행(디자인 루프 등) → 지난 회차 일감이 실제로 좋아졌는지 점수로 판정 → work/selfloop_log.json.
